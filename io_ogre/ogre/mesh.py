...
def dot_mesh( ob, path, force_name=None, ignore_shape_animation=False, normals=True, tangents=4, isLOD=False, **kwargs):
    ...
    target_file = os.path.join(path, '%s.lua' % obj_name )
    ...
    if logging:
        print('      - Generating:', '%s.lua' % obj_name)
    ...
    #with open(target_file, "w") as f:
    with open(target_file, "w") as luaFile:
        ...
        for F in mesh.polygons:
            ...
            for loop_idx, idx in zip(F.loop_indices, tri):
                ...
                r, g, b, a = vertex_color_lookup.get(loop_idx)
                ...
                '''
                vert = VertexNoPos(numverts, nx, ny, nz, r, g, b, a, vert_uvs)
                ...
                #    doc.leaf_tag('colour_diffuse', {'value' : '%6f %6f %6f %6f' % (r, g, b, a)})
                ...
                luaFile.write(f"\t\t--Vertex {vertIdx}\n")
                luaFile.write("\t\t{\n")
                luaFile.write(f"\t\t\ttexture = {{ u = {u:6f}, v = {v:6f} }},\n")
                luaFile.write(f"\t\t\tposition = {{ x = {x:6f}, y ={z:6f}, z = {-y:6f} }},\n")
                luaFile.write(f"\t\t\tcolor = {{ r = {r:6f}, g = {g:6f}, b = {b:6f}, a = {a:6f} }},\n")
                luaFile.write(f"\t\t\tnormal = {{ x = {nx:6f}, y = {ny:6f}, z = {nz:6f} }},\n")
                luaFile.write(f"\t\t\ttangent = {{ x = {tx:6f}, y = {ty:6f}, z = {tz:6f} }}")
                luaFile.write("\n\t\t},\n")
        ...
        for matidx, (mat_name, extern, mat) in enumerate(materials):
            ...
            luaFile.write("indices = \n{\n")
            #doc.start_tag('submesh', submesh_attributes)
            #doc.start_tag('faces', {
            #    'count' : str(len(material_faces[matidx]))
            #})
            triIdx = 0
            for fidx, (a, b, c) in enumerate(material_faces[matidx]):
                luaFile.write(f"\t{{ a = {a}, b = {b}, c = {c} }}, -- Triangle {triIdx}\n ")
                triIdx += 1
                #doc.leaf_tag('face', {
                #    'v1' : str(v1),
                #    'v2' : str(v2),
        ...
        if logging:
            print('      - Created .lua at', timer_diff_str(start), "seconds")
    ...
    logging.info('      - Created .lua in total time %s seconds', timer_diff_str(start))
